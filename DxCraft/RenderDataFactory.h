...
class RenderDataFactory
{
	...
	static Microsoft::WRL::ComPtr<ID3D11Buffer> CreateVertexBuffer(Graphics& gfx, Container ct);

	template<typename Container>
	static Microsoft::WRL::ComPtr<ID3D11Buffer> CreateIndexBuffer(Graphics& gfx, Container ct);

	static std::pair<Microsoft::WRL::ComPtr<ID3D11VertexShader>, Microsoft::WRL::ComPtr<ID3D11InputLayout>>
		CreateVSBlob(Graphics& gfx, const wchar_t* filePath, D3D11_INPUT_ELEMENT_DESC* ied, UINT ied_size);

	static Microsoft::WRL::ComPtr<ID3D11PixelShader> CreatePSBlob(Graphics& gfx, const wchar_t* filePath);
...
inline Microsoft::WRL::ComPtr<ID3D11Buffer>
	RenderDataFactory::CreateVertexBuffer(Graphics& gfx, Container ct)
{
...
template<typename Container>
inline Microsoft::WRL::ComPtr<ID3D11Buffer> RenderDataFactory::CreateIndexBuffer(Graphics& gfx, Container ct)
{
...
inline void RenderDataFactory::UpdateVScBuf(Graphics& gfx, Microsoft::WRL::ComPtr<ID3D11Buffer>& ptr, T cBuf)
{
	...
	if (ptr.Get() == nullptr) {
		...
		GFX_EXCEPT_INFO(gfx.getDevice()->CreateBuffer(&cbd, nullptr, &ptr));
	}

	D3D11_MAPPED_SUBRESOURCE msr;
	gfx.getContext()->Map(ptr.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &msr);
	memcpy(msr.pData, &ptr, sizeof(cBuf));
